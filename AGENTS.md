...
| Paramètre | Valeur |
|---|---|
| Montant journalier attendu | **5 000 F** (valeur en production depuis le 06/08/2026 ; toujours lue depuis `contrats.montant_journalier`, jamais codée en dur) |
...
1. **Enregistrer un versement = 1 tap.** Depuis le calendrier, toucher un jour dû non versé propose directement « Versement de 5 000 F » avec un seul bouton Valider.
2. **Zéro champ obligatoire évitable.** Date = aujourd'hui, montant = celui du contrat, mode = celui de la dernière fois.
